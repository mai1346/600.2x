...
class SimpleVirus(object):
    ...
    def reproduce(self, popDensity):
        ...
        reproducetest = random.random()
        if reproducetest <= self.maxBirthProb * (1 - popDensity):
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for i in range (numTrials):
        ...
        trial=[]
        for j in range(300):
            trial.append(P.update())  
        entire.append(trial)
    for x in range(300):
        subtotal=0
        for y in range(numTrials):
            subtotal+=entire[y][x]
        result.append(subtotal/numTrials)
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        for drug in mutresistances.keys():
            if random.random() <= self.mutProb:
                mutresistances[drug]= not self.resistances[drug]
...
class TreatedPatient(Patient):
    ...
    def getResistPop(self, drugResist):
        ...
        """
#        ResistPop=self.getViruses()[:]
#        for virus in ResistPop[:]:
#            for drug in drugResist:
#                if virus.isResistantTo(drug)==False:
#                    ResistPop.remove(virus)
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for i in range(numTrials):
        ...
        trial=[]
        for j in range(150):
            trial.append((P.update(),P.getResistPop(['guttagonol'])))
    ...
    for x in range(len(trial)):
        ...
        for y in range(numTrials):
            subpop+=entire[y][x][0]
            subresist+=entire[y][x][1]       
        population.append(subpop/numTrials)
        resistant.append(subresist/numTrials)
    pylab.plot(population, label = "Total Population")
